...
function Initialize 
{
	...
	RegisterHandler (CreateHandler "System.Int16" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.UInt16" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.Int32" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.UInt32" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.Int64" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.UInt64" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.IntPtr" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.UIntPtr" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.Single" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.Double" $null "DumpRawQuoted")
	RegisterHandler (CreateHandler "System.Decimal" $null "DumpRawQuoted")
	...
	else
	{
		...
		if ([string]::IsNullOrEmpty($resolved_type_name))
		{
			...
			if ($context.type_name.StartsWith($script:system_namespace))
			{
				...
				{
					$resolved_type_name = "string" # $context.type_name.Substring($script:system_namespace.Length)
				}
			}
		}
...
function DumpString([Hashtable]$dump_state, $context)
{
	...
	# escape embedded double-quote characters that might prematurely terminate the string
	$s = $s.Replace('"', '`"')
